...
impl Eval {
            ...
            Statement::Anew(i, v) => {
                ...
                if self.is_error(&val) {
                    ...
                    match store.get(&name) {
                        ...
                        None => Some(Object::Error(format!("identifier '{}' unknown", name))),
                    }
                }
            }
            Statement::Include(i) => {
    ...
    fn apply_function(&mut self, function: Expr, args: Vec<Object>) -> Object {
        let (params, body, store) = match self.eval_expr(function) {
            ...
            Some(o) => return Object::Error(format!(" The function '{}' is no where to be found", o)),
            None => return Object::Null,
        };
    ...
    fn eval_ident(&mut self, ident: Ident) -> Object {
        ...
        match self.store.borrow_mut().get(&i) {
            Some(i) => i,
            None => Object::Error(format!("unable to find the identifier '{}'", i)),
        }
    }
